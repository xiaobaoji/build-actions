...
export Op_name="DX4600"                # 修改主机名称为OpenWrt(填0为不作修改)
...
export DNS_Settings="114.114.114.114 223.5.5.5"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
export Customized_Information="小鲶鱼 $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
# 修改插件名字
sed -i 's/"MihomoTProxy"/"透明上网"/g' `egrep "MihomoTProxy" -rl ./`
