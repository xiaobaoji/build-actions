...
export Op_name="XiaomiR3G"                # 修改主机名称为XiaomiR3G(填0为不作修改)

# 内核和系统分区大小(不是每个机型都可用)
export Kernel_partition_size="256"            # 内核分区大小 (填写您想要的数值,数值以MB计算，填0为不作修改),如果你不懂就填0
export Root_partition_size="1024"              # 系统分区大小,每个机型默认值不一样 (填写您想要的数值,数值以MB计算，填0为不作修改),如果你不懂就填0
...
export OpenClash_Core="2"                   # 编译固件增加OpenClash时,把核心下载好,单核心为3MB左右大小(填1为下载单核{dev},2为下载三核{dev/meta/premium},0为不需要核心)
...
export AdGuardHome_Core="1"                  # 编译固件增加AdGuardHome时,把核心下载好,需要注意的是一个核心20多MB的,小闪存机子搞不来(1为启用命令,填0为不需要核心)
...
sed -i 's/"ShadowSocksR 客户端"/"科学上网"/g' `egrep "ShadowSocksR 客户端" -rl ./`
...
sed -i 's/"AdGuard Home"/"NAS"/g' `egrep "AdGuard Home" -rl ./`
